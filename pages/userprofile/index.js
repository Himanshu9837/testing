...
export default  function VendorProfile  ()  {
  ...
  const purchesitem = async () => {
    ...
    if (arrayOfData) {
      ...
      reatingbuyer(cart_dataa);

      let userId = await fetch(
        apiKey + "api/order/buyerorderlist/" + cart_dataa
      );
      userId = await userId.json();
  ...
  const solditem = async () => {
    const arrayOfData = localStorage.getItem("user");
    if (arrayOfData) {
      const d = arrayOfData !== null ? JSON.parse(arrayOfData) : [];
      const cart_dataa = d.tokenData.id;
      getsellerstatus(cart_dataa);
      badgecondition(cart_dataa);
      let userId = await fetch(
        apiKey + "api/order/sellerorderlist/" + cart_dataa
      );
      userId = await userId.json();
      // console.log(userId.orderlist[0].productId.images[0]);
      setSold(userId.orderlist);
    }
  };
  const getsellerstatus = async (cart_dataa) => {
  ...
  const badgecondition = async (cart_dataa) => {
    let sellerId = await fetch(apiKey + "api/badges/applybadges/" + cart_dataa);
    sellerId = await sellerId.json();
            ...
            <div className="change_style">
              <Box sx={{ bgcolor: "background.paper" }}>
                ...
                <div className="view_style">
                  <FlipCameraAndroidIcon onClick={changeui} />
                </div>
